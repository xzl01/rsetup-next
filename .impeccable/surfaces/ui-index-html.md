...
Package-mirror benchmarking extends the existing Soft UI with Test mirrors beside Preview changes and compact result rows that stack on narrow screens. It tests known provider IDs sequentially, reports completed/total progress, and lets Stop testing finish the current request before skipping remaining mirrors. Results show separate system and Radxa latency and small-index sample speed; the note states the 1 MiB-per-mirror limit and distinguishes samples from peak bandwidth, while demonstration runs carry a visible synthetic label. Testing never selects or applies a mirror automatically. Choosing a result updates the selector and clears the previous plan and acknowledgement, requiring a new preview before application. Request errors retain translatable details and re-render in the selected locale. Source-revision changes clear benchmark results and invalidate existing plans and pending previews; request-version checks prevent older responses from restoring stale state.
