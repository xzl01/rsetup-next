...
实现目标：
1. 通过 `libnvme` 静态链接读取 NVMe 磁盘设备与 SMART / Health 健康遥测数据。
...
| **阶段 3** | Libnvme 静态链接与原生采集后端 | `crates/rsetup-core/build.rs`<br>`crates/rsetup-core/src/nvme/sys.rs` | `nvme::tests::test_native_or_mock_nvme_read` |
...
4. 静态链接验证：编译出的二进制文件通过 `ldd` 检查，不强制动态依赖 `libnvme.so.1`。
